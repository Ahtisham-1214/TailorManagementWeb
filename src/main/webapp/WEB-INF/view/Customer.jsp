...
<div style="background: white; padding: 32px; border-radius: 12px; box-shadow: 0 8px 30px rgba(0, 0, 0, 0.08); width: 360px; border: 1px solid rgba(0, 0, 0, 0.05);">
    <h2 style="color: #2c3e50; margin-top: 0; margin-bottom: 24px; font-weight: 600; font-size: 22px;">Customer
        Information</h2>

    <!-- Display success or error message -->
    <% if (message != null && !message.isEmpty()) { %>
    <div style="color: <%= message.toLowerCase().contains("success") ? "green" : "red" %>; font-weight: bold; margin-bottom: 16px;">
        <%= message %>
    </div>
    <% } %>

    <!-- Add a form element with an ID -->
    <form id="customerForm" onsubmit="return validateForm(event)" action="OrderServlet" method="post">
        <div style="margin-bottom: 24px;">
            ...
            <input type="text" id="name" name="name" value="<%= name != null ? name : "" %>" required
                   style="width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 6px; box-sizing: border-box; font-size: 14px; transition: border-color 0.2s;"
                   placeholder="Name">
        </div>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 12px;">
            ...
            <button type="button" onclick="clearFields()"
                    style="padding: 10px 20px; border: 1px solid #e2e8f0; border-radius: 6px; cursor: pointer; font-weight: 500; background-color: white; color: #4a5568; transition: all 0.2s;">
                Clear
            </button>
            <button type="submit"
                    style="padding: 10px 20px; border: none; border-radius: 6px; cursor: pointer; font-weight: 500; background: linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%); color: white; transition: all 0.2s;">
                Save
            </button>
        </div>
    </form>
